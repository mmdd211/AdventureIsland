...
func _draw() -> void:
	...
	if compact:
		...
		return
	last_shown_zones = DataCatalog.map_order()

	var points := {}
	for zone_id in DataCatalog.map_order():
		points[zone_id] = DataCatalog.ui_position(str(zone_id)) + TILE_SIZE * 0.5
	for connection in DataCatalog.connections():
	...
	for zone_id in DataCatalog.map_order():
		_draw_zone_tile(Rect2(DataCatalog.ui_position(str(zone_id)), TILE_SIZE), str(zone_id), false)

func _draw_zone_tile(rect: Rect2, zone_id: String, show_player: bool) -> void:
	var metadata := DataCatalog.map_metadata(zone_id)
	if metadata.is_empty():
		# 未知/过期的 zone id（如旧存档遗留的区域 id）按未探索地块渲染。
		metadata = {"id": zone_id, "display_name": zone_id.capitalize(), "theme": {}, "is_boss": false}
	var theme: Dictionary = metadata.get("theme", {})
	var known := _is_known(zone_id)
	var current := zone_id == GameState.current_zone_id
	var sky := Color(str(theme.get("sky_top", "536c77"))) if known else Color("536c77")
	var ground := Color(str(theme.get("ground_grass", "65767a"))) if known else Color("65767a")
	var body := Color(str(theme.get("ground_body", "35464d"))) if known else Color("35464d")
	var accent := Color(str(theme.get("accent", "aab8b6"))) if known else Color("aab8b6")
	...
	if not compact:
		...
		_draw_centered_text(metadata.get("display_name", ""), rect.position + Vector2(rect.size.x * 0.5, rect.size.y + 9), 12, name_color)
		if bool(metadata.get("is_boss", false)):
			var seal_color := Color("ffd700") if GameState.is_boss_defeated(str(metadata.get("region_id", ""))) else Color("8a5a35")
...
func _draw_compact_zone_name(rect: Rect2, zone_id: String) -> void:
	var metadata := DataCatalog.map_metadata(zone_id)
	var display_name := str(metadata.get("display_name", zone_id.capitalize()))
	...
	_draw_centered_text(display_name, label_rect.get_center() + Vector2(0, -1), 12, Color("fffbe8"))
...
func _draw_motif_pixels(rect: Rect2, theme: Dictionary, known: bool) -> void:
	var accent := Color(str(theme.get("accent", "aab8b6"))) if known else Color(0.22, 0.27, 0.31)
	if not known:
	...
	var motif: String = theme.get("metadata", {}).get("motif", "")
	match motif:
		"bright_pastoral":
			for index in range(6):
		...
		"mushroom_canopy":
			for index in range(5):
				...
				draw_rect(Rect2(x, rect.position.y + 16.0, 7, 4), Color(str(theme.get("landmark_b", "d94837"))))
				draw_rect(Rect2(x + 2, rect.position.y + 4.0, 3, 13), Color(str(theme.get("landmark_a", "f2d7a6"))))
		"glowing_roots":
		...
		"mossy_arches":
			for index in range(4):
				...
				draw_rect(Rect2(x, rect.position.y + 8.0, 6, 12), Color(str(theme.get("landmark_a", "d9d2ea"))))
				draw_rect(Rect2(x - 2, rect.position.y + 6.0, 10, 3), Color(str(theme.get("landmark_b", "7bcf70"))))
		_:
